...
def update_frame(n):
    frame_files = sorted(os.listdir("received_frames"), key=lambda x: int(x.split('_')[1].split('.')[0]))
    if frame_files:
...
def serve_frame(filename):
    return send_file(os.path.join("received_frames", filename), mimetype='image/jpeg')
...
def run_server_and_client():
    ...
    client_process = subprocess.Popen(["python3", "client_cycle.py", "path_to_your_video.mp4"])
