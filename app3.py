...
ic()
def objectDetector(frame_path:str):
    if frame_path is None or not isinstance(frame_path, str):
        print("Invalid input video path")
        return

    img = frame.to_ndarray(format="bgr24")

    return av.VideoFrame.from_ndarray(img, format="brg24")
...
def run_server_and_client():
    ...
    client_process = subprocess.Popen(["python3", "client_cycle1.py", "video_1.py"])
